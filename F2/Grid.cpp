...
bool Grid::IsOverLapping(GameObject* Newobj)
{
	...
	if (pLadder != NULL)
	{
		...
		int h = pos.HCell();
		int v = pos.VCell();
		CellPosition pos2 = pLadder->GetEndPosition();
		int h2 = pos2.HCell();
		int v2 = pos2.VCell();
		...
		{
			if (CellList[i][h]->HasLadder()!=NULL)
			{
				ArrOfLadders[IndexOfArr] = CellList[i][h]->HasLadder();
				IndexOfArr++;
			}
			for (int j = 0;j < IndexOfArr;j++)
			{
				int H = (ArrOfLadders[j]->GetPosition()).HCell();
				int V = (ArrOfLadders[j]->GetPosition()).VCell();
...
void Grid::SaveAll(ofstream& OutFile, int type)
{
	...
	for (int i = NumVerticalCells - 1; i >= 0; i--)
	{
		for (int j = 0; j < NumHorizontalCells; j++)
		{
			...
			else if (dynamic_cast<CardTwelve*>(CellList[i][j]->HasCard()) != NULL && c12 > 0)
			{
				CardTwelve* a = dynamic_cast<CardTwelve*>(CellList[i][j]->HasCard());
				a->SpecialSave(OutFile, type);
			}
			...
			else if (CellList[i][j]->GetGameObject() != NULL)
			{
				...
				else if (dynamic_cast<CardTwelve*>(CellList[i][j]->HasCard()) != NULL)
				{
					c12++;
				}
