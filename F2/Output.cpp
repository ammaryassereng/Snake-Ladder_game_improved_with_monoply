...
int Output::GetCellStartX(const CellPosition & cellPos) const
{
	...
	int h = cellPos.HCell()*UI.CellWidth;
	return h; // this line should be changed with your implementation
}
...
{
	///TODO: implement the following function as described in Output.h file
	int v = (cellPos.VCell()*UI.CellHeight)+UI.ToolBarHeight;
	return v; // this line should be changed with your implementation
}
...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_DELETE_GAME_OPJECT] = "images\\Menu_Delete_Game_Object.jpg";
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_INPUT_DICE] = "images\\Menu_Input_Dice_Value.jpg";
	MenuItemImages[ITM_NEW_GAME] = "images\\Menu_New_Game.jpg";
	MenuItemImages[ITM_EXIT_FROM_PLAY_MODE] = "images\\Menu_Exit_From_Play_Mode.jpg";
...
void Output::DrawLadder(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	int X1 = fromCell.HCell();
	int Y1 = fromCell.VCell();
	int num1 = fromCell.GetCellNum();
	int X2 = toCell.HCell();
	int Y2 = toCell.VCell();
	int num2 = toCell.GetCellNum();
	if (!(X1 == X2))
	{
		return;
	}
	...
	for (int i = GetCellStartY(toCell)+UI.CellHeight;i <= GetCellStartY(fromCell);i = i + UI.CellHeight)
	{
		pWind->SetPen(UI.LadderColor, UI.LadderlineWidth);
...
void Output::DrawSnake(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	pWind->DrawPolygon(X, Y, NumOfPoints);
	delete[]X;
	delete[]Y;
}
